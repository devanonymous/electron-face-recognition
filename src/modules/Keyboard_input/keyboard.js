class Keyboard {
    constructor(options) {
        ...
        this.input = document.querySelector(options.input) || document.querySelector(".input-text");
    ...
    /**
     *
     * @param {string} input - QuerySelector, for example - '.example' or '#example'
     */
    changeInput(input) {
    ...
    keyboardClickHandler(event) {
        ...
        if (element) {
            ...
            if (this.debug) {
                console.log("это не кнопка", element);
            }
    ...
    textChanged(text) {
        if (this.debug) {
            console.log("текст в кастомный эвент -> ", text);
        }
